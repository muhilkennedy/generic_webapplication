...
import com.platform.tenant.config.TenantDataSource;
...
public class CityController {
    ...
    @Autowired
    TenantDataSource tenantDs;
    
    @Autowired
    DbFlywayConfiguration db;
    
    @Autowired
    FlywayDataSourceProperties dsp;
...
//        return new ResponseEntity<>(cities, HttpStatus.OK);
    	...
    	map.put("jdbcUrl","jdbc:mysql://localhost:3306/platformdb3?useSSL=false&allowPublicKeyRetrieval=true");
    	map.put("driverClassName","com.mysql.jdbc.Driver");
    	map.put("username","root");
    	map.put("password","root@123");
    	map.put("schema","platformdb3");
    	dsp.addNewDs("tenant3", map);
    	db.migrate();
    	tenantDs.createAndAddNewDataSourceInRunTime("tenant3", map);
