...
public class EmployeeServiceImpl implements EmployeeService {
	...
	@Override
	public User updateProfilePic(MultipartFile multipartFile) throws Exception {
		User user = (User) BaseSession.getUser();
		File file = new File(FileUtil.getTempDirectory() + user.getRootId()
				+ FileUtil.getFileExtension(multipartFile.getOriginalFilename()));
		file.mkdirs();
		multipartFile.transferTo(file);
		byte[] image = ImageUtil.getThumbnailImage(Files.readAllBytes(file.toPath()),
				FileUtil.getFileExtensionFromName(multipartFile.getOriginalFilename()));
		FileUtils.writeByteArrayToFile(file, image);
		String piciurl = StorageService.getStorage().saveFile(file, StorageUtil.GCS_EMPLOYEE_PROFILE_PATH);
	...
	@Override
	public User updateUserLocale(String locale) {
		Employee emp = (Employee)BaseSession.getUser();
		emp.setLocale(locale);
		return (User) employeeDao.save(emp);
	}
