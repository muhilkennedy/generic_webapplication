...
create table if not exists tenant (rootid bigint NOT NULL AUTO_INCREMENT PRIMARY KEY, active BOOL DEFAULT TRUE, timecreated bigint DEFAULT 0, timeupdated bigint DEFAULT 0, modifiedby bigint DEFAULT 0, createdby bigint DEFAULT 0, version bigint DEFAULT 0, locale varchar(12) default 'en_US', timezone varchar(32) default 'IST', name varchar(50) NOT NULL, uniquename varchar(50) NOT NULL UNIQUE, parent bigint);
...
create table if not exists tenantsubscription (rootid bigint NOT NULL AUTO_INCREMENT PRIMARY KEY, tenantid bigint NOT NULL, active BOOL DEFAULT TRUE, timecreated bigint DEFAULT 0, timeupdated bigint DEFAULT 0, modifiedby bigint DEFAULT 0, createdby bigint DEFAULT 0, version bigint DEFAULT 0, startdate datetime, enddate datetime, CONSTRAINT FOREIGN KEY(tenantid) REFERENCES tenant(rootid));
